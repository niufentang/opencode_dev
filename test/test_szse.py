...
用法：
    python test/test_szse.py                           # 全部栏目 × 全部分页 × 全部下载
    python test/test_szse.py --category 数据接口        # 仅采集指定栏目
    python test/test_szse.py --max-pages 2 --download-n 10    # 限制页数与下载数
    python test/test_szse.py --no-download              # 仅采集元数据，不下载
    python test/test_szse.py --download-only            # 仅从已有元数据下载

日志输出：log/test_szse.log（同时输出到控制台）

分页说明：技术公告 36 页、数据接口 5 页、技术指南 3 页，其余 1 页
"""
...
logging.basicConfig(
    ...
    datefmt="%H:%M:%S",
    handlers=[
        logging.FileHandler(LOG_DIR / "test_szse.log", encoding="utf-8"),  # 写盘
        logging.StreamHandler(),                                            # 输出到终端
    ],
)
logger = logging.getLogger("test_szse")   # 模块级 logger，API 层用 __name__ 自动继承此配置
...
def main() -> None:
    ...
    parser.add_argument("--max-pages", type=int, help="每栏目最大页数 (默认全部)")
