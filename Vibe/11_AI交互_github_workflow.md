...
4. 使用 Python 3.12，启用 pip 缓存
5. 通过 pip install -r requirements.txt 安装依赖
6. 运行命令：python pipeline/pipeline.py --sources sse,szse,chinaclear --step collect --limit 10 --per-category-limit 5 
7. 支持多个 LLM 密钥（LLM_PROVIDER、DEEPSEEK_API_KEY、DASHSCOPE_API_KEY、MOONSHOT_API_KEY）
