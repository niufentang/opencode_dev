...
<span style="color:red">[删除] 删除新订单申报中申报数量字段取值小于</span>
<span style="color:red">1000000000</span> <span style="color:red">的限制说明。</span>
...
交易网关数据接口规范
4.4.2 平台状态 PlatformState..........................................................................................36
4.4.3 执行报告分区信息 ExecRptInfo............................................................................36
4.4.4 分区序号同步 ExecRptSync.................................................................................. 36
4.4.5 分区序号同步响应 ExecRptSyncRsp....................................................................37
...
总体示意图如下：
3.2.1 业务类型
订单申报需要指定业务类型（ ApplID ）， <span style="color:red">其产生的回报以不同的执行报告分区</span>
<span style="color:red">（</span> <span style="color:red">PartitionNo</span> <span style="color:red">）划分为多个逻辑上相互独立的数据流。</span> 根据具体业务的不同，下表给出 <span style="color:red">业务</span>
<span style="color:red">与</span> 业务类型 <span style="color:red">、分区</span> 的对应关系，并明确业务相关属性。
业务类型
<span style="color:red">执行报告分区</span>
业务
支持撤单
申报确认
成交确认
...
交易网关数据接口规范
3 、市转限订单（ OrdType=3 ）、本方最优（ OrdType=4 ）、对手方最优（ OrdType=5 ）
消息流如下：
14
...
属于同一个分区 4 ，从而在同一个流中按序发送 <span style="color:red">；同一个业务也可以属于多个分区，通过多</span>
<span style="color:red">个执行报告流发送</span> 。
OMS 与 TDGW 建立会话后， TDGW 会向 OMS 推送执行报告分区信息（ ExecRptInfo ）
消息，其中包含 PBU 列表和分区列表， OMS 应根据此信息维护多个逻辑上的执行报告流。
<span style="color:red">对于分区列表信息，由后台交易系统确定，可能随后台优化而调整。</span> <span style="color:red">OMS</span> <span style="color:red">应避免自行配置</span>
<span style="color:red">固定分区信息，影响系统的灵活性和兼容性。</span>
OMS 与 TDGW 建立会话后，应根据 ExecRptInfo 中的信息，向 TDGW 发送各个执行报
4 <span style="color:red">比如，发行（</span> <span style="color:red">BizID=300010</span> <span style="color:red">）和配股</span> <span style="color:red">/</span> <span style="color:red">科创板配售（</span> <span style="color:red">BizID=300020</span> <span style="color:red">）均属于</span> <span style="color:red">PartitionNo</span> <span style="color:red">=991</span> <span style="color:red">的分区。</span>
18
...
格
式
HHMMSSsssnnnn ， HH 为小时，取值
22

交易网关数据接口规范
范围 00-23 ， MM 为分钟，取值范围
00-59 ， SS 为秒，取值范围 00-59 ， sss
为毫秒，取值范围 000-999 ， nnnn 为
百纳秒，取值 0000-9999
Boolean
C1
代表该字符串内容为布尔值，有效取
...
会话发起方，如需序号重置，则需设置 ResetSeqNumFlag 为 ’Y’ 、
24

交易网关数据接口规范
NextExpectedMsgSeqNum 为 1.
...
段填写空格。前 5 位有效。
5.
“结算会员代码”字段： B 股结算会员代码，对于非交易业务取值无意义，对于 A 股
投资者取值无意义。对于 B 股境外投资者 C9 类帐户此记录不能为空，直接填写中登公
30

交易网关数据接口规范
司公布的 B 股结算会员代码，不足 5 位的左侧补 0 。对于 B 股境内投资者 C1 类帐户无
意义。前 5 位有效。
6.
...
方投资者账户”字段：前 10 位有效；
“发起方营业部代码”字段：前 5 位有效； DestSecurity
字段：前 6 位有效。
<span style="color:red">8.</span>
<span style="color:red">OrderQty</span> <span style="color:red">字段：取值小于</span> <span style="color:red">1000000000</span> <span style="color:red">；</span>
8.
ClOrdID 字段：会员内部订单编号，长度必须等于 10 个字符，各字符仅允许填写数字
0-9 、大小写字母 a-z A-Z ，不允许其他字符。
9.
OwnerType 暂不启用。
10. Text 字段：第 23 至 32 位，仅供港交所使用，用于填写沪股通业务的 BCAN 编码（香
...
<span style="color:red">3.</span>
<span style="color:red">TotalValueTraded</span> <span style="color:red">字段：如果实际成交金额超过999,999,999.99999</span> <span style="color:red">元，则该字段返回</span>
...
中 Pbu 可能为 OMS 所连接 TDGW 上的登录 PBU ，也可能为该 TDGW 上订阅的其他 PBU
（仅包含订阅成功的 PBU ）， TDGW 在该循环体中首先给出登录 PBU ，后给出订阅的其他
PBU （如有）。 <span style="color:red">对于分区列表信息，由后台交易系统确定，可能随后台优化而调整。</span> <span style="color:red">OMS</span>
<span style="color:red">应避免自行配置固定分区信息，影响系统的灵活性和兼容性。</span>
...
<metadata>
{
  "title": "20260123_IS122_上海证券交易所交易网关STEP接口规格说明书（竞价平台）0",
  "source_url": null,
  "raw_path": "knowledge\\raw\\sse\\测试文档\\20260123_IS122_上海证券交易所交易网关STEP接口规格说明书（竞价平台）0.58版_20260122（新竞价新综业技术开发稿）.pdf",
  "markdown_path": "knowledge\\articles\\sse\\markdown\\测试文档\\IS122_上海证券交易所交易网关STEP接口规格说明书（竞价平台）0.58版_20260122（新竞价新综业技术开发稿.md",
  "file_hash": "sha256:d0f561cbcf315720731a0700decf70124b55fd88b7f490679ac64bb0f7b765c2",
  "file_format": "pdf",
  "page_count": 42,
  ...
  "parse_date": "2026-05-02T01:48:28.132960+00:00",
  "sub_category": null
}
</metadata>