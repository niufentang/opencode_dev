...
交易网关 STEP 接口规格说明书
（互联网交易平台固收迁移技术开发稿）
2. <span style="color:blue">11</span> 版
二〇二 <span style="color:blue">六</span> 年 <span style="color:blue">二</span> 月
...
率的合理性说明
1. 3.2.2.2 询价报价处理消息流图和 3.2.2.3 报价处理消息流图中
调整笔误，将报价回复响应中的 ClOrdID 调整为 QuoteMsgID
2. 4.4.2.2 询价请求响应、 4.4.2.3 转发询价请求、 4.4.2.5 报价状态
回报、 4.4.2.6 转发报价、 4.4.2.8 转发报价回复、 4.4.3.2 转发意向
申报、 4.4.4.2 转发成交申报中重复组中增加登录或订阅交易单元
3. 4.4.3 意向申报类、 4.4.4 成交申报类中对于 ContractMultiplier
为保证上下游定义统一，调整为 N5(2)
4. 4.4.4.3 成交申报响应中对于订单拒绝码，调整为 TrdAckStatus
=8 时有效
2025.11
2.10
开发稿
5. 4.4.1.1 新订单申报中对于固收各业务添加对 Text 字段的选填
说明
6. 4.4.2.2 询价请求响应、 4.4.2.5 报价状态回报、 4.4.4.3 成交申报
响应、 4.4.4.4 成交确认、 4.4.5.1 执行报告中对于不同业务返回字
段进行了明确区分
7. 4.4.2 询价报价业务类、 4.4.3 意向申报类、 4.4.4 成交申报类中
调整了部分字段的前后顺序
8. 针对本次固收迁移业务，取消营业部代码字段
<span style="color:blue">1.</span> <span style="color:blue">4.4.4.4</span> <span style="color:blue">成交确认中添加了场务应急成交录入的内容</span>
<span style="color:blue">2.</span> <span style="color:blue">4.4.4.1</span> <span style="color:blue">成交申报（</span> <span style="color:blue">Trade</span> <span style="color:blue">Capture</span> <span style="color:blue">Report,</span> <span style="color:blue">MsgType=AE)</span> <span style="color:blue">买入价</span>
<span style="color:blue">格和卖出价格对于合并申报明确表示中间方买入或卖出的价格</span>
<span style="color:blue">3.</span> <span style="color:blue">3.2.2.2</span> <span style="color:blue">询价报价处理消息流图中对于一债通询价明确每个交</span>
<span style="color:blue">2026.02</span>
<span style="color:blue">2.11</span>
<span style="color:blue">开发稿</span>
<span style="color:blue">易参与人仅可回复一次</span>
4. <span style="color:blue">4.4.2.3</span> <span style="color:blue">转发询价请求（</span> <span style="color:blue">Allege</span> <span style="color:blue">Quote</span> <span style="color:blue">Request,</span> <span style="color:blue">MsgType=R</span> <span style="color:blue">）、</span>
<span style="color:blue">4.4.2.6</span> <span style="color:blue">转发报价（</span> <span style="color:blue">Allege</span> <span style="color:blue">Quote,</span> <span style="color:blue">MsgType=S</span> <span style="color:blue">）、</span> <span style="color:blue">4.4.2.8</span> <span style="color:blue">转发报价</span>
<span style="color:blue">回复（</span> <span style="color:blue">Allege</span> <span style="color:blue">Quote</span> <span style="color:blue">Response,</span> <span style="color:blue">MsgType=AJ</span> <span style="color:blue">）当用户选择‘匿名’</span>

日期
版本
状态
说明
<span style="color:blue">时，相关字段进行隐藏</span>
...
2 系统简介
2.1 系统接入
为满足业务发展需求和提升交易服务水平，本所通过交易网关（ TDGW ）对接互联网交易平台和新固
定收益系统，提供实时交易流接口。 TDGW 对接交易系统及市场参与者系统（ OMS ）的示意图如下：
...
报一笔报价回复，交易系统对报价及报价回复进行撮合成交并发送成交回报执行报告。若询价方不接受报
价，报价在询价失效后也自动失效。
询价未成交前，询价方可以撤销询价。报价未成交前，报价方可以撤销报价。对于基金通询价，若报
价方不进行报价，询价在超时后会自动失效，整个交易过程结束；若询价交易超过总时长仍未成交，则需
通过询价请求响应及报价状态回报将失效消息转发至相关方。
此消息流适用于：基金通询价、一债通询价和待定报价。 <span style="color:blue">一债通询价不支持撤销报价后重新回复，每</span>
...
3.2.2.5 成交申报消息流图
3.2.2.5.1 协议配对申报模式
协议配对申报模式指交易双方各申报一笔订单，订单内容包括对手方信息、买卖方向和约定号等，当
双方信息匹配后成交，订单未匹配前可以撤销。
此模式适用于现券协商成交。
OMS1
TDGW
OMS2
...
户（ E 字头）申报。
4. OrderQty 表示划转数量 , 填写非 0 正整数，允许零散股；营业部
代码必填。
1. SecurityID 表示转入的标的产品代码。
2. Price 固定填 1 。
...
融资融券担保品划入
（ E 字头）申报。
4. OrderQty 划转数量 , 填写非 0 正整数，允许零散股；营业部代码
必填。
1. SecurityID 表示转入的标的产品代码。
2. Price 固定填 1 。
3.Side 固定填 2 。卖出转义为标的证券从“证券公司信用交易担保证
...
金账户、投资者中国结算交易账户、销售人代码、券商网点号码、营业部代码为必填。
2 、撤单申报中， APPIID 、发起方投资者账户、发起方业务交易单元号、发起方三方回购专用账户、
发起方三方回购专户对应交易单元号、发起方交易员一债通账户、银行间托管账号、投资者中国结算开放
式基金账户、投资者中国结算交易账户、销售人代码、券商网点、 SecurityID 、 Side 取值应与原申报相同，
OrigClOrdID 的取值应与待撤原订单的 ClOrdID 相同， Text 字段选填。对于开放式基金、要约 / 现金选择权、
...
权、融资融券非交易业务暂不启用。
5 、发起方营业部代码对开放式基金、要约 / 现金选择权、融资融券非交易业务、双非可转债转股冻结、
私募可交换债换股、债券回售（一债通）、债券回售撤销、债券转托管、三方回购转入转出和竞买业务暂
不启用，其他业务必填。
4.4.2 询价报价业务类
...
户端编码或网点号码。
说明：
1 、对于询价请求订单申报， TransactTime 选填，其他非必填字段要求如下，未提及的非必填字段均无
效：
业务类型（ ApplID ）
申报填写说明
撤单填写说明
...
N
C12
参与方个数，取值 =9 ，后接重复组，依次包含
询价发起方的登录或订阅交易单元、投资者账
...
448
PartyID
发起方登录或订阅交易单元（除基金通外必填） N
C8
交易单
取 17 ，表示当前 PartyID 的取值为登录或订阅
452
PartyRole
N
N4
...
户端编码或网点号码。
说明：
1. 对于以下业务，发起方交易单元、发起方交易员一债通账户、 TransactTime 、 ExecMethod 有效； 申
报或撤单成功时， OrderID 有效；订单被拒绝时， QuoteRequestRejectReason 有效。其他非必填字段要求如下，
未提及的非必填字段均无效：
业务类型
申报填写说明
撤单
（ ApplID ）
待定报价
NoCounterpartyParticipant/CounterpartyParticipantCode 、
1. 撤单成功时 OrderQty 或 LastQty
（ 600200 ）
表示剩余数量，否则无效。
OrderQty 、 LastQty 同申报信息
2. OrigClOrdID 同申报信息
一债通询价
NoCounterpartyParticipant/CounterpartyParticipantCode 、
1. 撤单成功时 OrderQty 表示剩余
（ 600190 ）
数量，否则无效。
OrderQty 同申报信息
2. OrigClOrdID 同申报信息
...
N
ntime
参与方个数，取值 =8 ，后接重复组，依次包含登录或
订阅交易单元、询价发起方的投资者账户、交易员一
...
码或网点号码。
说明：
1 、基金通询价交易（ 600022 ）时，投资者中国结算开放式基金账户、投资者中国结算交易账户、销
售人代码、券商网点号码为必填。
2 、一债通询价（ 600190 ）、待定报价（ 600200 ）时，发起方交易员一债通账户、对手方机构代码必
填。如询价方选择‘匿名’ <span style="color:blue">且为‘净额结算’</span> 时，发起方交易员一债通账户、发起方投资者账户（如有）
填‘ anonymous ’。
3 、转发信息字段与被转发字段相同，对于业务无效字段，不进行转发。
...
=0
=0
3 、对于确定报价、一债通询价和待定报价，发起方交易员一债通账户必填， TransactTime 选填，其
他非必填字段要求如下，未提及的非必填字段对该业务无效：
业务类型（ ApplID ）
申报填写说明
撤单填写说明
...
买，此处应当填 OfferSize
4 、对于确定报价或待定报价的双边报价，如在限定范围内发布，将拆分为两笔子订单转发给对手方；
如全市场发布，则拆分为两笔子订单通过逐笔行情对外发布。
61
...
N
ntime
参与方个数，取值 =9 ，后接重复组，依次包含报价
发起方的登录或订阅交易单元、投资者账户、业务
...
编码或网点号码。
说明：
1 、对于基金通询价， SecurityID （ 48 ）左数前六位有效， ExecID （ 17 ）左数前 10 位有效，且均不补空格。
2 、对于以下业务，响应中的 QuoteID 字段，如申报为报价，填写交易所唯一化处理后的报价请求 ID ；如
申报为报价回复，且回复中填写了 QuoteID ，则响应中与申报时填写字段相同。发起方交易单元、发起方
交易员一债通账户、 TransactTime 、 QuoteStatus 、 ExecMethod 有效； 申报或撤单成功时， OrderID 有效；订
单被拒绝时， QuoteRequestRejectReason 有效。其他非必填字段要求如下，未提及的非必填字段对该业务无
效：
业务类型
报价回报填写说明
报价回复回报填写说明
...
OfferSize 有效表示剩余
/
QuoteReqID 、 BidPx 、 BidSize 、
OfferPx 、 OfferSize 同申报信息
（ 600190 ）
数量，否则无效。
待定报价
/
QuoteReqID 、 BidSize 、 OfferSize 同
（ 600200 ）
申报信息
64
...
N
ntime
参与方个数，取值 =8 ，后接重复组，依次包含登录
或订阅交易单元、报价发起方的投资者账户、交易
...
编码或网点号码。
说明：
1 、确定报价（ 600180 ）、一债通询价（ 600190 ）、待定报价（ 600200 ）时，发起方交易员一债通账
户、对手方机构代码必填。如询价方或报价方选择‘匿名’ <span style="color:blue">且为‘净额结算’</span> 时，发起方交易员一债通账
户、发起方投资者账户（如有）填‘ anonymous ’。
2 、转发信息字段与被转发字段相同，对于业务无效字段，不进行转发。
...
价方选择‘匿名’ <span style="color:blue">且为‘净额结算’</span> 时，发起方交易员一债通账户、发起方投资者账户（如有）填‘ anonymous ’。
2 、转发信息字段与被转发字段相同，对于业务无效字段，不进行转发。
71
...
代码
说明：
1 、转发信息字段与被转发字段相同，对于业务无效字段，不进行转发。
74
...
一债通账户
说明：
非必填字段填写说明， QuoteRejectReason 当订单申报拒绝时有效，未提及的非必填字段对该业务无效：
业务类型（ ApplID ）
申报
撤单
协议回购意向申报
IOIRefID 、 SecurityID 同申报
SecurityID 、 Price 同申报信息
信息
（ 600150 ）
三方回购意向申报
发起方投资者账户、 Price 同申报信息
IOIRefID 同申报信息
（ 600160 ）
债券现券意向申报
IOIRefID 、 SecurityID 、发起
（ 600170 ）
SecurityID 、 Price 、发起方投资者账户同申报信息
方投资者账户同申报信息
76
...
N
price
合并申报时代表 <span style="color:blue">中间方</span> 买入价格
640
Price2
申报价格 2
N
...
上海证券交易所交易网关 STEP 接口规范（互联网交易平台）
合并申报时代表 <span style="color:blue">中间方</span> 卖出价格（元）
455
SecurityAltID
辅助证券代码
N
...
户、业务单元、投资者账户、对手方交易员一债通
453
NoPartyIDs
Y
N2
账户 1 、对手方交易员一债通账户 2 。取值为 5
448
PartyID
发起方交易员一债通账户
Y
...
N4
单元号。
发起方投资者账户， TradeReportType 为 0 或 2 时
448
PartyID
N
C13
...
注 3 ：对于债券借贷到期续做（ TrdType: 303 ）或三方回购到期续做（ TrdType: 143 ），仅生成一笔成交，
该笔成交包含原合约到期结算及新开合约信息。
2 、业务撤单或确认或拒绝填写说明，未提及的非必填字段对该业务无效
ApplID
TrdType
描述
撤单
...
C600
说明：
1. 对于协议回购批量申报 N 笔质押券校验通过后，将生成 N 笔转发成交申报给对手方，每笔仅有一笔
质押券。
2. 对于 OrigTradeID ，撤单时必填，表示被撤消订单的交易所订单编号；对于 ExecRefID ，
TradeReportType 为 1 时，对非首期合约表示原成交编号；为 3 时表示被拒绝订单的交易所订单编号；其他
场景无效。证券账户名称（与投资者账户一一对应，如对应账户未查询到名称，则无此字段）有效。其他
转发信息字段与被转发字段相同，对于业务无效字段，不进行转发。
...
2 、各业务响应字段说明如下，发起方业务交易单元、发起方投资者账户、 Text 字段与原始申报相同，
其他未提及的非必填字段均无效 :
(1) 业务发起
ApplID
TrdType
...
131
协商成交
NoUnderlyings 填 1 ， Price 、 SecurityID 、 OrderQty 、 ShareProperty 、
RestrictedMonth 、 ContractMultiplier 、 GrossTradeAmt 同申报信息
132
到期确认
NoUnderlyings 、 SecurityID 、 OrderQty 同申报信息
协议回购
（ 600130 ）
133
到期续做
NoUnderlyings 、 Price 、 SecurityID 、 OrderQty 、 ShareProperty 、
RestrictedMonth 、 ContractMultiplier 、 GrossTradeAmt 同申报信息
94
...
134
提前终止
NoUnderlying 、 SecurityID 、 OrderQty 、 Price 同申报信息
135
换券申报
NoUnderlyings 、 EventType 、 SecurityID 、 OrderQty 、 ShareProperty 、
ContractMultiplier 、 GrossTradeAmt 同申报信息
136
解除质押
NoUnderlyings 、 SecurityID 、 OrderQty 同申报信息
141
协商成交
Price 、 BasketID 、 NoUnderlyings 、 SecurityID 、 OrderQty 同申报信
息
142
到期购回
/
...
145
换券申报
NoUnderlyings 、 EventType 、 SecurityID 、 OrderQty 、 TrdMatchID 同
申报信息
146
解除质押
/
147
补券申报
NoUnderlyings 、 EventType 、 SecurityID 、 OrderQty 同申报信息
成交
Price 、 NoUnderlyings 、 SecurityID 、 OrderQty 同申报信息
现券协商成
交（ 600210 ）
不适用
现券协商
...
301
协商成交
1. Price 、 SecurityAltID 、 DeliveryQty 同申报信息
2. NoUnderlyings 、 SecurityID 、 OrderQty 同申报信息
302
到期结算
DeliveryForm 、 SettlDeliveryType 同申报信息
303
到期续做
1. Price 、 SecurityAltID 、 DeliveryQty 同申报信息
2. NoUnderlyings 、 SecurityID 、 OrderQty 、 EventType 、 TrdMatchID
同申报信息
304
提前终止
DeliveryForm 、 SettlDeliveryType 同申报信息
...
质押券变
更申报
NoUnderlyings 、 SecurityID 、 OrderQty 、 EventType 、 TrdMatchID 同
申报信息
306
解除质押
/
...
131
协商成交
OrigTradeID 、 TradeReport
协议回购
NoUnderlyings 、 SecurityID 、 Even
RefID 有效。对于协议回
tType 同申报信息（指换入券）
133
到期续做
...
上海证券交易所交易网关 STEP 接口规范（互联网交易平台）
交， NoUnderlyings 填 1 ， S
135
换券申报
ecurityID 、 EventType 同申
136
解除质押
报信息（指换入券）。
141
...
305
质押券变更申报
（ 3 ）业务发起、撤单、确认或拒绝失败， TradeReportRejectReason 、 Text 有效，撤单时 TradeReportRefID
有效 :
ApplID
描述
TrdType 、 NoUnderlyings 、 SecurityID 有效；换券时，仅返
协议回购（ 600130 ）
回换入券
三方回购（ 600140 ）
TrdType 有效
合并申报（ 600220 ）
NoUnderlyings 、 SecurityID 有效
现券协商成交（ 600210 ）
债券借贷（ 600300 ）
TrdType 有效
96
...
资者账户名称
说明：
1 、对于成交报告申报模式的被动成交方或协议回购到期续做中的前期合约了结，也将收到成交确认。
其中：申报来源（ ExecMethod ）填 0 ，订单所有者类型（ OwnerType ）填‘ 103 ’；会员内部订单编号
（ TradeReportID ）按照网页端申报自动生成；登录或订阅交易单元、业务交易单元同首期申报数据，如首
期本方通过网页端申报，则此成交实时仍仅供网页端查看。
2 、各业务响应字段说明如下，证券账户名称（如对应账户未查询到名称，则无此字段）、 OrderEntryTime
有效， Text 字段与原始申报相同， LastPx 与申报中的 Price/Price2 （如有）相同， LastQty 与申报中的 OrderQty
（如有）相同，其他未提及的非必填字段均无效 :
ApplID
TrdType
描述
填写说明
...
131
协商成交
NoUnderlyings 填 1 ， LastPx 、 SecurityID 、 LastQty 、 ShareProperty 、
RestrictedMonth 、 ContractMultiplier 、 GrossTradeAmt 、 ExpirationDays
同申报信息
132
到期确认
NoUnderlyings 、 SecurityID 、 LastQty 、 OrigTradeDate 、 ExecRefID
同申报信息
NoUnderlyings 、 LastPx 、 SecurityID 、 LastQty 、 ShareProperty 、
133
到期续做
RestrictedMonth 、 ContractMultiplier 、 GrossTradeAmt 、 ExpirationDays 、
OrigTradeDate 、 ExecRefID 同申报信息
协议回购
（ 600130 ）
134
提前终止
NoUnderlying 、 SecurityID 、 LastQty 、 LastPx 、 ExpirationDays 、
OrigTradeDate 、 ExecRefID 同申报信息
NoUnderlyings 填 1 ， EventType 、 SecurityID 、 LastQty 、 ShareProperty 、
135
换券申报
ContractMultiplier 、 GrossTradeAmt 表示换入券信息， OrigTradeDate 、
ExecRefID 同申报信息
136
解除质押
NoUnderlyings 、 SecurityID 、 LastQty 、 OrigTradeDate 、 ExecRefID
...
141
协商成交
LastPx 、 TotalValueTraded 、 BasketID 、 NoUnderlyings 、 SecurityID 、
LastQty 、 ExpirationDays 同申报信息
142
到期购回
OrigTradeDate 、 ExecRefID 同申报信息
143
到期续做
LastPx 、 TotalValueTraded 、 BasketID 、 ExpirationDays 、 OrigTradeDate 、
ExecRefID 同申报信息
144
提前终止
LastPx 、 ExpirationDays 、 OrigTradeDate 、 ExecRefID 同申报信息
三方回购
（ 600140 ）
145
换券申报
NoUnderlyings 、 EventType 、 SecurityID 、 LastQty 、 TrdMatchID 、
OrigTradeDate 、 ExecRefID 同申报信息
146
解除质押
OrigTradeDate 、 ExecRefID 同申报信息
147
补券申报
NoUnderlyings 、 EventType 、 SecurityID 、 LastQty 、 OrigTradeDate 、
ExecRefID 同申报信息
成交
现券协商成
交（ 600210 ）
...
不适用
合并申报
LastPx 、 SettlType 、 NoUnderlyings 、 SecurityID 、 LastQty 同申报信
息， TotalValueTraded 按照价格数量进行计算
...
债券借贷
（ 600300 ）
1. LastPx 、 TotalValueTraded 、 SecurityAltID 、 DeliveryQty 、
ExpirationDays 同申报信息
2. NoUnderlyings 、 SecurityID 、 LastQty 、 NoDistribInsts 、
CashDistribAgentAcctNumber 、 CashDistribAgentAcctName 、
CashDistribAgentName 、 CashDistribAgentCode 同申报信息
302
到期结算
DeliveryForm 、 SettlDeliveryType 、 OrigTradeDate 、 ExecRefID 同申
报信息
101
...
到期续做
1. LastPx 、 TotalValueTraded 、 SecurityAltID 、 DeliveryQty 、
ExpirationDays 、 OrigTradeDate 、 ExecRefID 同申报信息
2. NoUnderlyings 、 SecurityID 、 LastQty 、 EventType 、 TrdMatchID 、
NoRelatedSym 、 NoDistribInsts 、 CashDistribAgentAcctNumber 、
CashDistribAgentAcctName 、 CashDistribAgentName 、
CashDistribAgentCode 、 ContraLegRefID 、 SettlDeliveryType 同申报
信息
304
提前终止
DeliveryForm 、 SettlDeliveryType 、 TotalValueTraded 、 OrigTradeDate 、
ExecRefID 同申报信息
质押券变
NoUnderlyings 、 SecurityID 、 LastQty 、 EventType 、 TrdMatchID 、
305
更申报
OrigTradeDate 、 ExecRefID 同申报信息
306
...
452
PartyRole
取 39 ，表示当前 PartyID 的取值为对手方投资者账户
N
N4
账户
对手方
...
C180
证券账
取 36 ，表示当前 PartyID 的取值为对手方投资者账户
452
PartyRole
N
N4
...
CxlQty 撤单数量
无意义
4 、对于下列业务，除协议必填字段外，申报来源字段有效，发起方交易员一债通账户、 Text 字段同
申报信息，其他非必填字段说明如下，未提及的非必填字段对该业务无效：
ApplID
业务类型
申报成功
申报失败
...
600200
待定报价
* 如竞买预约时勾选了自动发起，且未再手动发起竞买预约，系统将在相应时点自动发起竞买申报。
此时竞买预约发起方将收到竞买申报执行报告。此时会员内部订单编号（ TradeReportID ）由系统自动
生成；申报来源（ ExecMethod ）、订单所有者类型（ OwnerType ）、登录或订阅交易单元、业务交易
单元同预约数据。如预约时通过网页端申报，登录或订阅单元将填写交易员绑定的交易单元；如交易
员未绑定，则此执行报告仅供网页端查看。
* 对于一债通各类现券非匹配成交（点击成交、询价成交和竞买成交等），将在成交回报中返回对手
方的一债通账户、证券账户和账户名称信息。如结算方式为净额结算且任意一方选择‘匿名’，则此
字段将填写‘ anonymous ’；如结算方式为净额结算且双方均显名或者结算方式‘ RTGS ’，则将发送
实际的交易信息；但如账户名称无法自动加载，将无此字段。
4.4.5.1.2 撤单失败执行报告
标签
字段名
字段描述
...
点号码。
说明：
1. 发起方营业部代码字段对于开放式基金、要约 / 现金选择权、融资融券非交易及固收迁移业务暂不启
用。
...
消息数据错误
Message Data Error
注：本表仅提供交易网关错误码，系统后台错误码参照互联网交易平台 / 新固定收益系统错误码信息文
件。
118
...
<metadata>
{
  "title": "20260424_IS122_上海证券交易所交易网关STEP接口规格说明书（互联网交易平台）2",
  "source_url": null,
  "raw_path": "knowledge\\raw\\sse\\测试文档\\20260424_IS122_上海证券交易所交易网关STEP接口规格说明书（互联网交易平台）2.11版_20260424（新固收技术开发稿）.pdf",
  "markdown_path": "knowledge\\articles\\sse\\markdown\\测试文档\\IS122_上海证券交易所交易网关STEP接口规格说明书（互联网交易平台）2.11版_20260424（新固收技术开发稿.md",
  "file_hash": "sha256:230bed901afa3ac5efa3f96411cd72ba46cfb6ce8ceab0d5803d2ab074e89993",
  "file_format": "pdf",
  "page_count": 130,
