上海证券交易所
交易网关 STEP 接口规格说明书
（互联网交易平台 <span style="color:blue">/</span> <span style="color:blue">新固定收益系统</span> ）
<span style="color:blue">2.00</span> 版
二〇二 <span style="color:blue">六</span> 年 <span style="color:blue">五</span> 月
...
1.14
正式稿
1. 基于《 IS101 上海证券交易所竞价撮合平台市场参与者接口规格
说明书》文档正式废止，相关内容迁移至《 IS124 上海证券交易
所市场数据文件交换接口规格说明书》，更新相关引用描述。
<span style="color:blue">补充了新固定收益系统相关业务申报接口内容，包括</span> <span style="color:blue">:</span>
...
<span style="color:blue">2026.05</span>
<span style="color:blue">2.00</span>
<span style="color:blue">正式稿</span>
...
4.4.5 执行报告类 .......................................................................................................... 101
4.4.6 网络密码服务（ Password Service, MsgType = U006 ） ...................................109
4.4.7 其它消息 ...............................................................................................................111
附录 .................................................................................................................................................115
附录一计算校验和 .............................................................................................................. 115
附录二 PBU 及说明 ..............................................................................................................116
...
上海证券交易所交易网关 STEP 接口规范（互联网交易平台）
3.2 申报与回报
OMS 进行的新订单申报（ New Order Single ）、询价请求（ Quote Request ）、报价（ Quote ） <span style="color:blue">、意向申</span>
<span style="color:blue">报（</span> <span style="color:blue">IOI</span> <span style="color:blue">）和成交申报（</span> <span style="color:blue">Trade Capture Report</span> <span style="color:blue">）</span> 时，本所交易系统会进行前置检查，若检查未通过将返回订
单拒绝（ Order Reject ）、询价请求响应（ Quote Request Ask ）和报价状态回报（ QuoteStatusReport ） <span style="color:blue">、意</span>
<span style="color:blue">向申报响应（</span> <span style="color:blue">IOI Response</span> <span style="color:blue">）和成交申报响应（</span> <span style="color:blue">Trade Capture Report Response</span> <span style="color:blue">）</span> 消息。
对于通过前置校验的申报，交易系统根据业务的不同，向 OMS 返回相应的执行报告（ Execution Report ）、
转发询价请求（ Allege Quote Request ） <span style="color:blue">、</span> 转发报价（ Allege Quote ） <span style="color:blue">、转发意向申报（</span> <span style="color:blue">Allege IOI</span> <span style="color:blue">）和转发</span>
<span style="color:blue">成交申报（</span> <span style="color:blue">Allege Trade Capture Report</span> <span style="color:blue">）</span> 消息。执行报告包括对申报的确认，如对新订单的确认或拒绝响
应 1 、撤单响应等； <span style="color:blue">新订单申报及询价请求</span> 产生成交时，执行报告中会包含成交确认。
总体示意图如下：
3.2.1 业务类型
...
<span style="color:blue">此消息流适用于：基金通询价、一债通询价和待定报价。一债通询价不支持撤销报价后重新回复，每</span>
<span style="color:blue">个参与人仅可回复一次。</span>
...
<span style="color:blue">协议配对申报模式指交易双方各申报一笔订单，订单内容包括对手方信息、买卖方向和约定号等，当</span>
<span style="color:blue">双方信息匹配后成交，订单未匹配前可以撤销。</span>
...
<span style="color:blue">此模式适用于协议回购和三方回购的协商成交、提前终止、换券、到期续做和解除质押申报，以及债</span>
...
为月，取值范围 01-12 ， DD 为日，取值范围 01-31 <span style="color:blue">。</span>
ntime
C13
当前时区时间，格式 HHMMSSsss ， HH 为小时，取值范围 00-23 ， MM 为分钟，
取值范围 00-59 ， SS 为秒，取值范围 00-59 ， sss 为毫秒，取值范围 000-999 <span style="color:blue">。</span>
Boolean
C1
...
448
PartyID
发起方投资者 <span style="color:blue">账户</span>
Y
C13
账户
452
PartyRole
取 5 ，表示当前 PartyID 的取值为发起方投资者 <span style="color:blue">账</span>
Y
N4
...
<span style="color:blue">银行间托管账</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
...
3. <span style="color:blue">OrderQty</span> <span style="color:blue">必填；</span> <span style="color:blue">Text</span> <span style="color:blue">选填。</span>
<span style="color:blue">600270</span>
<span style="color:blue">三方回购转入转出</span>
<span style="color:blue">4.</span> <span style="color:blue">其他非必填字段无效。</span>
...
<span style="color:blue">4.</span> <span style="color:blue">MinQty</span> <span style="color:blue">如选择多主体中标时必填，否则不可填；</span> <span style="color:blue">Text</span> <span style="color:blue">选填；其他</span>
<span style="color:blue">非必填字段无效。</span>
...
<span style="color:blue">相同，本次不可修改；</span> <span style="color:blue">Text</span> <span style="color:blue">选填；其他非必填字段无效。</span>
1. <span style="color:blue">BidTransType</span> <span style="color:blue">填</span> <span style="color:blue">3</span> <span style="color:blue">；</span>
...
<span style="color:blue">SecondaryOrderID</span> <span style="color:blue">必填，表示应价对应的竞买申报编号；</span> <span style="color:blue">Text</span> <span style="color:blue">选填；</span>
<span style="color:blue">其他非必填字段无效。</span>
...
7 、参与方个数应小于等于 <span style="color:blue">14</span> ，重复组个数应 <span style="color:blue">与</span> 申报参与方个数相匹配并按序依次填写，非必填参与
...
448
PartyID
发起方投资者 <span style="color:blue">帐户账户</span>
Y
C13
发起方投
资者账户
452
PartyRole
取 5 ，表示当前 PartyID 的取值为发起方投资者 <span style="color:blue">帐户账户</span> 。
Y
N4
发起方业
448
...
<span style="color:blue">管账号</span>
<span style="color:blue">452</span>
<span style="color:blue">PartyRole</span>
<span style="color:blue">取</span> <span style="color:blue">28</span> <span style="color:blue">，表示当前</span> <span style="color:blue">PartyID</span> <span style="color:blue">的取值为银行间托管账号</span>
<span style="color:blue">N</span>
...
<span style="color:blue">发起方三方回购专户对应交易单元号、发起方交易员一债通账户、银行间托管账号、</span> 投资者中国结算开放
...
448
PartyID
询价发起方投资者 <span style="color:blue">帐户账户</span>
Y
C13
投资者
...
N4
账户
者 <span style="color:blue">帐户账户</span>
发起方
...
户端编码或网点号码。
说明：
<span style="color:blue">1</span> <span style="color:blue">、对于询价请求订单申报，</span> <span style="color:blue">TransactTime</span> <span style="color:blue">选填，其他非必填字段要求如下，未提及的非必填字段均无</span>
...
<span style="color:blue">撤单填写说明</span>
<span style="color:blue">基</span>
<span style="color:blue">金</span>
<span style="color:blue">通</span>
<span style="color:blue">询</span>
...
<span style="color:blue">OrigClOrdID</span>
<span style="color:blue">（</span> <span style="color:blue">600022</span> <span style="color:blue">）</span>
<span style="color:blue">必填</span>
<span style="color:blue">2.</span> <span style="color:blue">投资者中国结算开放式基金账户、投资者中国结算交易账户、销售</span>
<span style="color:blue">人代码、券商网点号码、营业部代码为必填</span>
...
<span style="color:blue">人代码</span> <span style="color:blue">”</span> <span style="color:blue">字段必须为空，表示向市场该产品对应全部做市商发起询价</span>
...
<span style="color:blue">发起方交易员</span>
<span style="color:blue">2.</span> <span style="color:blue">NoCounterpartyParticipant</span> <span style="color:blue">必填，可填</span> <span style="color:blue">[0,5]</span> <span style="color:blue">，</span> <span style="color:blue">0</span> <span style="color:blue">表示发送给全市场；如</span>
<span style="color:blue">信息必填</span>
<span style="color:blue">指定范围发送，</span> <span style="color:blue">CounterpartyParticipantCode</span> <span style="color:blue">必填</span>
...
<span style="color:blue">均需填写；否则仅填写对应方向的价格和数量即可</span>
50
...
<span style="color:blue">3.</span> <span style="color:blue">NoCounterpartyParticipant</span> <span style="color:blue">必填，可填</span> <span style="color:blue">[0,5]</span> <span style="color:blue">，填</span> <span style="color:blue">0</span> <span style="color:blue">时表示发给潜在投</span>
<span style="color:blue">资者；大于</span> <span style="color:blue">0</span> <span style="color:blue">时，</span> <span style="color:blue">CounterpartyParticipantCode</span> <span style="color:blue">必填</span>
...
户、业务交易单元代码、营业部代码、 <span style="color:blue">交易员</span>
453
NoPartyIDs
Y
N2
<span style="color:blue">一债通账户、</span> 投资者中国结算开放式基金账户、
...
N4
账户
<span style="color:blue">帐户账户</span>
发起方
询价发起方业务交易单元代码，填写 5 位业务
448
PartyID
...
<span style="color:blue">2.</span> <span style="color:blue">OrigClOrdID</span> <span style="color:blue">同申报信息</span>
<span style="color:blue">2.</span> <span style="color:blue">对于双边报价撤单，如双边订单中有一笔撤销成功，一笔因已成交或其他原因撤销失败，返回撤销</span>
<span style="color:blue">成功（已撤销）；如两笔均因已成交或其他原因撤销失败，返回撤销失败（已拒绝）。</span>
54
...
<span style="color:blue">订阅交易单元、</span> 询价发起方的投资者账户、 <span style="color:blue">交易员一</span>
<span style="color:blue">债通账户、对手方交易参与人机构代码、发起方</span> 投资
453
NoPartyIDs
Y
N2
...
C13
发起方投
取 5 ，表示当前 PartyID 的取值为发起方投资者 <span style="color:blue">帐户</span>
452
PartyRole
资者账户
N
...
码或网点号码。
说明：
<span style="color:blue">1</span> <span style="color:blue">、</span> 基金通询价交易 <span style="color:blue">（</span> <span style="color:blue">600022</span> <span style="color:blue">）</span> 时，投资者中国结算开放式基金账户、投资者中国结算交易账户、销
售人代码、券商网点号码为必填。
...
<span style="color:blue">填。如询价方选择‘匿名’时，发起方交易员一债通账户、发起方投资者账户（如有）填‘</span> <span style="color:blue">anonymous</span> <span style="color:blue">’。</span>
...
448
PartyID
报价发起方投资者 <span style="color:blue">帐户账户</span>
Y
C13
发起方投
取 5 ，表示当前 PartyID 的取值为发起方投资者 <span style="color:blue">帐户账</span>
452
PartyRole
资者账户
Y
...
<span style="color:blue">户、销售人代码、券商网点号码、营业部代码为必填。</span>
...
=0
=0
<span style="color:blue">3</span> <span style="color:blue">、对于确定报价、一债通询价和待定报价，发起方交易员一债通账户必填，</span> <span style="color:blue">TransactTime</span> <span style="color:blue">选填，其</span>
<span style="color:blue">他非必填字段要求如下，未提及的非必填字段对该业务无效：</span>
<span style="color:blue">业务类型（</span> <span style="color:blue">ApplID</span> <span style="color:blue">）</span>
<span style="color:blue">申报填写说明</span>
<span style="color:blue">撤单填写说明</span>
<span style="color:blue">确定报价（</span> <span style="color:blue">600180</span> <span style="color:blue">）</span> <span style="color:blue">1.</span> <span style="color:blue">SettlType</span> <span style="color:blue">、</span> <span style="color:blue">FullAmountTrade</span> <span style="color:blue">、</span> <span style="color:blue">PreTradeAnonymity</span> <span style="color:blue">必填</span>
<span style="color:blue">1.</span> <span style="color:blue">BidSize</span> <span style="color:blue">和</span>
<span style="color:blue">2.</span> <span style="color:blue">NoCounterpartyParticipant</span> <span style="color:blue">必填，可填</span> <span style="color:blue">[0,5]</span> <span style="color:blue">，</span> <span style="color:blue">0</span> <span style="color:blue">表示发送给全市场；</span>
<span style="color:blue">OfferSize</span> <span style="color:blue">参照说</span>
<span style="color:blue">如指定范围发送，</span> <span style="color:blue">CounterpartyParticipantCode</span> <span style="color:blue">必填</span>
<span style="color:blue">明</span> <span style="color:blue">2</span> <span style="color:blue">填写</span>
<span style="color:blue">3.</span> <span style="color:blue">DisplayQty</span> <span style="color:blue">选填，需要为最小变动单位的正整数倍；</span> <span style="color:blue">DisplayQty</span>
<span style="color:blue">2.</span> <span style="color:blue">OrigClOrdID</span>
<span style="color:blue">有效</span>
<span style="color:blue">和</span> <span style="color:blue">FullAmountTrade=1</span> <span style="color:blue">不可同时申报。如指定范围发送，不支持冰</span>
<span style="color:blue">山订单</span>
<span style="color:blue">4.</span> <span style="color:blue">支持双边报价：</span> <span style="color:blue">BidPx/BidSize</span> <span style="color:blue">和</span> <span style="color:blue">OfferPx/OfferSize</span> <span style="color:blue">需要至少填</span>
60

上海证券交易所交易网关 STEP 接口规范（互联网交易平台）
<span style="color:blue">写一对</span>
<span style="color:blue">一</span>
<span style="color:blue">债</span>
<span style="color:blue">通</span>
<span style="color:blue">询</span>
...
<span style="color:blue">（</span> <span style="color:blue">600190</span> <span style="color:blue">）</span>
<span style="color:blue">2.</span> <span style="color:blue">BidPx/BidSize</span> <span style="color:blue">和</span> <span style="color:blue">OfferPx/OfferSize</span> <span style="color:blue">需要填写其中一对，例如询</span>
<span style="color:blue">价请求中</span> <span style="color:blue">Side</span> <span style="color:blue">为买，此处应当填</span> <span style="color:blue">OfferPx</span> <span style="color:blue">和</span> <span style="color:blue">OfferSize</span>
<span style="color:blue">待定报价（</span> <span style="color:blue">600200</span> <span style="color:blue">）</span> <span style="color:blue">1.</span> <span style="color:blue">QuoteReqID</span> <span style="color:blue">必填</span>
<span style="color:blue">2.</span> <span style="color:blue">BidSize</span> <span style="color:blue">或</span> <span style="color:blue">OfferSize</span> <span style="color:blue">中的任一个必填，如如询价请求中</span> <span style="color:blue">Side</span> <span style="color:blue">为</span>
<span style="color:blue">买，此处应当填</span> <span style="color:blue">OfferSize</span>
<span style="color:blue">4</span> <span style="color:blue">、对于确定报价或待定报价的双边报价，如在限定范围内发布，将拆分为两笔子订单转发给对手方；</span>
<span style="color:blue">如全市场发布，则拆分为两笔子订单通过逐笔行情对外发布。</span>
...
交易单元代码、营业部代码、 <span style="color:blue">交易员一债通账户、</span>
453
NoPartyIDs
Y
N2
...
C13
投资者
取 5 ，表示当前 PartyID 的取值为发起方投资者 <span style="color:blue">帐</span>
452
PartyRole
Y
N4
账户
<span style="color:blue">户账户</span>
62
...
<span style="color:blue">或订阅交易单元、</span> 报价发起方的投资者账户、 <span style="color:blue">交易</span>
<span style="color:blue">员一债通账户、对手方交易参与人机构代码、发起</span>
453
NoPartyIDs
Y
N2
<span style="color:blue">方的</span> 投资者中国结算开放式基金账户、投资者中国
结算交易账户、销售人代码、券商网点号码。
...
<span style="color:blue">户、对手方机构代码必填。如询价方或报价方选择‘匿名’时，发起方交易员一债通账户、发起方投资者</span>
<span style="color:blue">账户（如有）填‘</span> <span style="color:blue">anonymous</span> <span style="color:blue">’。</span>
...
发起方投 448
PartyID
<span style="color:blue">报价回复方</span> 投资者 <span style="color:blue">帐户账户</span>
Y
C13
67
...
端编码或网点号码。
说明：
1 、 <span style="color:blue">对于</span> 基金通询价交易 <span style="color:blue">（</span> <span style="color:blue">600022</span> <span style="color:blue">）</span> ，投资者中国结算开放式基金账户、投资者中国结算交易账户、
销售人代码、券商网点号码 <span style="color:blue">、营业部代码</span> 为必填 <span style="color:blue">；点击成交时报价请求编号（</span> <span style="color:blue">QuoteID</span> <span style="color:blue">）必填，匹配成交</span>
<span style="color:blue">时询价请求编号（</span> <span style="color:blue">QuoteReqID</span> <span style="color:blue">）必填；报价回复类型（</span> <span style="color:blue">QuoteRespType</span> <span style="color:blue">）仅可填</span> <span style="color:blue">1</span> <span style="color:blue">，如未接受，超时自动失</span>
<span style="color:blue">效</span> 。
68
...
<span style="color:blue">填，</span> <span style="color:blue">TransactTime</span> <span style="color:blue">选填，其他非必填字段要求如下，未提及的非必填字段对该业务无效：</span>
<span style="color:blue">业务类型（</span> <span style="color:blue">ApplID</span> <span style="color:blue">）</span>
...
<span style="color:blue">N</span>
<span style="color:blue">ntime</span>
<span style="color:blue">参与方个数，取值</span> <span style="color:blue">=7</span> <span style="color:blue">，后接重复组，依次包含登录</span>
<span style="color:blue">或订阅交易单元、报价回复方的投资者账户、交易</span>
<span style="color:blue">员一债通账户、投资者中国结算开放式基金账户、</span>
<span style="color:blue">453</span>
<span style="color:blue">NoPartyIDs</span>
<span style="color:blue">Y</span>
<span style="color:blue">N2</span>
<span style="color:blue">投资者中国结算交易账户、销售人代码、券商网点</span>
<span style="color:blue">号码。</span>
<span style="color:blue">登录或订阅交易单元（指转发消息的接收方</span> <span style="color:blue">,</span> <span style="color:blue">除基金</span>
<span style="color:blue">登录或订</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">N</span>
...
<span style="color:blue">N4</span>
<span style="color:blue">单元。</span>
<span style="color:blue">发起方投</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">报价回复方投资者账户</span>
<span style="color:blue">N</span>
<span style="color:blue">C13</span>
<span style="color:blue">资者账户</span>
<span style="color:blue">452</span>
<span style="color:blue">PartyRole</span>
<span style="color:blue">取</span> <span style="color:blue">5</span> <span style="color:blue">，表示当前</span> <span style="color:blue">PartyID</span> <span style="color:blue">的取值为发起方投资者账户</span>
<span style="color:blue">N</span>
<span style="color:blue">N4</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
...
<span style="color:blue">通账户</span>
<span style="color:blue">一债通账户</span>
<span style="color:blue">投资者中</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">投资者场外开放式基金账户</span>
<span style="color:blue">N</span>
<span style="color:blue">C12</span>
<span style="color:blue">国结算开</span>
...
<span style="color:blue">放式基金</span>
<span style="color:blue">452</span>
<span style="color:blue">PartyRole</span>
<span style="color:blue">N</span>
<span style="color:blue">N4</span>
...
<span style="color:blue">国结算交</span>
...
<span style="color:blue">N</span>
<span style="color:blue">N4</span>
<span style="color:blue">易账户</span>
...
<span style="color:blue">销售人代</span>
...
<span style="color:blue">N</span>
<span style="color:blue">C9</span>
<span style="color:blue">券商网点</span>
...
<span style="color:blue">说明：</span>
<span style="color:blue">1</span> <span style="color:blue">、一债通询价（</span> <span style="color:blue">600190</span> <span style="color:blue">）、待定报价（</span> <span style="color:blue">600200</span> <span style="color:blue">）时，发起方交易员一债通账户必填。如询价方或报</span>
<span style="color:blue">价方选择‘匿名’时，发起方交易员一债通账户、发起方投资者账户（如有）填‘</span> <span style="color:blue">anonymous</span> <span style="color:blue">’。</span>
<span style="color:blue">2</span> <span style="color:blue">、转发信息字段与被转发字段相同，对于业务无效字段，不进行转发。</span>
71
...
<span style="color:blue">N</span>
<span style="color:blue">N65(2)</span>
...
上海证券交易所交易网关 STEP 接口规范（互联网交易平台）
<span style="color:blue">账户、投资者账户、业务交易单元。取值为</span> <span style="color:blue">3</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
...
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">发起方投资者账户</span>
<span style="color:blue">N</span>
<span style="color:blue">C13</span>
<span style="color:blue">取</span> <span style="color:blue">5</span> <span style="color:blue">，表示当前</span> <span style="color:blue">PartyID</span> <span style="color:blue">的取值为发起方投资者账</span>
<span style="color:blue">→</span>
<span style="color:blue">452</span>
<span style="color:blue">PartyRole</span>
<span style="color:blue">N</span>
...
<span style="color:blue">说明：</span>
<span style="color:blue">非必填字段填写说明，未提及的非必填字段对该业务无效：</span>
<span style="color:blue">业务类型（</span> <span style="color:blue">ApplID</span> <span style="color:blue">）</span>
...
<span style="color:blue">三方回购意向申报</span>
<span style="color:blue">2.</span> <span style="color:blue">发起方投资者账户必填，若为正回购方，填三方回购专用账</span>
...
<span style="color:blue">债券现券意向申报</span>
<span style="color:blue">SecurityID</span> <span style="color:blue">、</span>
...
<span style="color:blue">Y</span>
<span style="color:blue">ntime</span>
<span style="color:blue">发起方重复组，取值</span> <span style="color:blue">=3</span> <span style="color:blue">，依次包含登录或订阅交易单元、</span>
<span style="color:blue">发起方的交易员一债通账户、对手方交易参与人机构代</span>
<span style="color:blue">453</span>
<span style="color:blue">NoPartyIDs</span>
<span style="color:blue">Y</span>
<span style="color:blue">N2</span>
...
<span style="color:blue">订单拒绝码，</span> <span style="color:blue">ExecType=8</span> <span style="color:blue">时有效</span>
<span style="color:blue">N</span>
...
<span style="color:blue">Y</span>
<span style="color:blue">C16</span>
<span style="color:blue">参与方个数，取值</span> <span style="color:blue">=4</span> <span style="color:blue">，后接重复组，依次包含登录</span>
<span style="color:blue">或订阅交易单元、发起方业务交易单元、交易员一</span>
<span style="color:blue">453</span>
<span style="color:blue">NoPartyIDs</span>
<span style="color:blue">Y</span>
<span style="color:blue">N2</span>
<span style="color:blue">债通账户、投资者账户。</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
...
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">发起方投资者账户，协议回购意向申报时不适用</span>
<span style="color:blue">N</span>
<span style="color:blue">C13</span>
<span style="color:blue">→</span>
<span style="color:blue">452</span>
...
<span style="color:blue">N</span>
<span style="color:blue">price</span>
<span style="color:blue">合并申报时代表中间方买入价格</span>
<span style="color:blue">640</span>
<span style="color:blue">Price2</span>
...
上海证券交易所交易网关 STEP 接口规范（互联网交易平台）
<span style="color:blue">合并申报时代表中间方卖出价格（元）</span>
...
<span style="color:blue">折算比例（</span> <span style="color:blue">%</span> <span style="color:blue">）</span>
<span style="color:blue">N</span>
<span style="color:blue">N5(2)</span>
<span style="color:blue">→</span>
...
<span style="color:blue">户、业务单元、投资者账户、对手方交易员一债通</span>
<span style="color:blue">453</span>
<span style="color:blue">NoPartyIDs</span>
<span style="color:blue">Y</span>
<span style="color:blue">N2</span>
<span style="color:blue">账户</span> <span style="color:blue">1</span> <span style="color:blue">、对手方交易员一债通账户</span> <span style="color:blue">2</span> <span style="color:blue">。取值为</span> <span style="color:blue">5</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">发起方交易员一债通账户</span>
<span style="color:blue">Y</span>
...
<span style="color:blue">N4</span>
<span style="color:blue">单元号。</span>
<span style="color:blue">发起方投资者账户，</span> <span style="color:blue">TradeReportType</span> <span style="color:blue">为</span> <span style="color:blue">0</span> <span style="color:blue">或</span> <span style="color:blue">2</span> <span style="color:blue">时</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">N</span>
<span style="color:blue">C13</span>
...
<span style="color:blue">N4</span>
<span style="color:blue">员一债通账户</span>
<span style="color:blue">对手方交易员一债通账户</span> <span style="color:blue">2</span> <span style="color:blue">，仅合并申报发起时有</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">N</span>
<span style="color:blue">C10</span>
<span style="color:blue">效表示卖方交易员。其他申报无意义。</span>
<span style="color:blue">→</span>
<span style="color:blue">452</span>
<span style="color:blue">PartyRole</span>
...
<span style="color:blue">说明：</span>
<span style="color:blue">1</span> <span style="color:blue">、业务申报填写说明，未提及的非必填字段对该业务无效</span>
<span style="color:blue">ApplID</span>
<span style="color:blue">TrdType</span>
...
<span style="color:blue">数量；如质押券为公募</span> <span style="color:blue">REITs</span> <span style="color:blue">，</span> <span style="color:blue">ShareProperty</span> <span style="color:blue">必填，限售份额不可</span>
<span style="color:blue">续做；</span>
...
<span style="color:blue">3.</span> <span style="color:blue">OrderRestrictions</span> <span style="color:blue">、</span> <span style="color:blue">BasketID</span> <span style="color:blue">必填</span>
<span style="color:blue">4.</span> <span style="color:blue">NoUnderlyings</span> <span style="color:blue">选填，可填</span> <span style="color:blue">[1,3]</span> <span style="color:blue">，</span> <span style="color:blue">SecurityID/OrderQty</span> <span style="color:blue">填写对应质</span>
<span style="color:blue">押券代码和数量，质押券应当属于已选择的篮子</span>
<span style="color:blue">5.</span> <span style="color:blue">TotalAccruedInterestAmt</span> <span style="color:blue">（回购利息）和</span> <span style="color:blue">TotalSettlCurrAmt</span> <span style="color:blue">（到期</span>
<span style="color:blue">结算金额）必填，均为正数，保留两位小数，四舍五入，其中：</span>
<span style="color:blue">回购利息</span> <span style="color:blue">=</span> <span style="color:blue">成交金额</span> <span style="color:blue">×</span> <span style="color:blue">回购利率</span> <span style="color:blue">/100×</span> <span style="color:blue">实际占款天数</span> <span style="color:blue">/365</span>
<span style="color:blue">到期结算金额</span> <span style="color:blue">=</span> <span style="color:blue">成交金额</span> <span style="color:blue">+</span> <span style="color:blue">回购利息</span>
6. <span style="color:blue">发起方投资者账户必填，填三方回购专用账户；对手方接受时填</span>
<span style="color:blue">写普通账户</span>
<span style="color:blue">7.</span> <span style="color:blue">Text</span> <span style="color:blue">、</span> <span style="color:blue">ConfirmID</span> <span style="color:blue">、</span> <span style="color:blue">Memo</span> <span style="color:blue">选填</span>
...
<span style="color:blue">6.</span> <span style="color:blue">DeliveryQty</span> <span style="color:blue">选填，当</span> <span style="color:blue">DeliveryForm =101</span> <span style="color:blue">时必填，表示归还标的</span>
<span style="color:blue">券数量</span>
...
<span style="color:blue">描述</span>
<span style="color:blue">填写说明</span>
<span style="color:blue">7.</span> <span style="color:blue">UAInterestAccrualDays</span> <span style="color:blue">必填，表示实际借贷期限，</span> <span style="color:blue">MaturityDate</span>
...
<span style="color:blue">1.</span> <span style="color:blue">SettlDeliveryType</span> <span style="color:blue">必填</span>
<span style="color:blue">2.</span> <span style="color:blue">DeliveryForm</span> <span style="color:blue">必填，表示逾期结算方式</span>
<span style="color:blue">3.</span> <span style="color:blue">ExecRefID</span> <span style="color:blue">必填</span>
<span style="color:blue">4.</span> <span style="color:blue">SettlDeliveryType</span> <span style="color:blue">必填，</span> <span style="color:blue">SettlDeliveryType=2</span> <span style="color:blue">时，资金账户、资</span>
<span style="color:blue">金账户名称、资金开户行、支付系统行号必填</span>
...
<span style="color:blue">注</span> <span style="color:blue">3</span> <span style="color:blue">：对于债券借贷到期续做（</span> <span style="color:blue">TrdType: 303</span> <span style="color:blue">）或三方回购到期续做（</span> <span style="color:blue">TrdType: 143</span> <span style="color:blue">），仅生成一笔成交，</span>
<span style="color:blue">该笔成交包含原合约到期结算及新开合约信息。</span>
<span style="color:blue">2</span> <span style="color:blue">、业务撤单或确认或拒绝填写说明，未提及的非必填字段对该业务无效</span>
<span style="color:blue">ApplID</span>
<span style="color:blue">TrdType</span>
<span style="color:blue">描述</span>
<span style="color:blue">撤单</span>
...
<span style="color:blue">N</span>
<span style="color:blue">date</span>
<span style="color:blue">TradeReportType</span> <span style="color:blue">为</span> <span style="color:blue">1</span> <span style="color:blue">时，对非首期合约表</span>
<span style="color:blue">示原成交编号；为</span> <span style="color:blue">3</span> <span style="color:blue">时表示被拒绝订单的</span>
<span style="color:blue">19</span>
<span style="color:blue">ExecRefID</span>
<span style="color:blue">N</span>
<span style="color:blue">C16</span>
...
<span style="color:blue">N</span>
<span style="color:blue">C96</span>
<span style="color:blue">发起方重复组，依次包含登录或订阅交易</span>
<span style="color:blue">单元、发起方的交易员一债通账户、投资</span>
<span style="color:blue">者账户、投资者账户名称，对手方交易参</span>
<span style="color:blue">453</span>
<span style="color:blue">NoPartyIDs</span>
<span style="color:blue">Y</span>
<span style="color:blue">N2</span>
<span style="color:blue">与人代码以及对手方交易员信息。取值为</span>
<span style="color:blue">6</span> <span style="color:blue">。</span>
...
<span style="color:blue">场景无效。证券账户名称（与投资者账户一一对应，如对应账户未查询到名称，则无此字段）有效。其他</span>
...
<span style="color:blue">对于合并申报中间方发起时，取值为</span> <span style="color:blue">0</span>
<span style="color:blue">44</span>
<span style="color:blue">Price</span>
<span style="color:blue">申报价格</span>
<span style="color:blue">N</span>
...
<span style="color:blue">订单拒绝码，</span> <span style="color:blue">TrdAckStatus=8</span> <span style="color:blue">时有效</span>
<span style="color:blue">N</span>
...
<span style="color:blue">N</span>
<span style="color:blue">C32</span>
<span style="color:blue">参与方个数，取值</span> <span style="color:blue">=4</span> <span style="color:blue">，后接重复组，依次包含登</span>
<span style="color:blue">录或订阅交易单元、发起方业务交易单元、交易</span>
<span style="color:blue">453</span>
<span style="color:blue">NoPartyIDs</span>
<span style="color:blue">Y</span>
<span style="color:blue">N2</span>
<span style="color:blue">员一债通账户、投资者账户。</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">发起方登录或订阅交易单元。</span>
<span style="color:blue">Y</span>
...
<span style="color:blue">N</span>
<span style="color:blue">C32</span>
<span style="color:blue">参与方个数，取值</span> <span style="color:blue">=7</span> <span style="color:blue">，后接重复组，依次</span>
...
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">对手方投资者账户</span>
<span style="color:blue">Y</span>
<span style="color:blue">C13</span>
<span style="color:blue">→</span>
...
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">证券账户名称，支持中文。</span>
<span style="color:blue">N</span>
<span style="color:blue">C180</span>
<span style="color:blue">→</span>
...
<span style="color:blue">N</span>
<span style="color:blue">N4</span>
<span style="color:blue">资者账户名称</span>
<span style="color:blue">说明：</span>
...
<span style="color:blue">2</span> <span style="color:blue">、各业务响应字段说明如下，证券账户名称（如对应账户未查询到名称，则无此字段）、</span> <span style="color:blue">OrderEntryTime</span>
...
<span style="color:blue">场务应急成</span>
<span style="color:blue">不适用</span>
<span style="color:blue">场务应急</span>
<span style="color:blue">成交录入</span>
<span style="color:blue">交录入</span>
<span style="color:blue">（</span> <span style="color:blue">600310</span> <span style="color:blue">）</span>
<span style="color:blue">301</span>
<span style="color:blue">协商成交</span>
...
<span style="color:blue">申报来源（仅对固收迁移业务有效）</span>
<span style="color:blue">2405</span>
<span style="color:blue">ExecMethod</span>
<span style="color:blue">N</span>
<span style="color:blue">C1</span>
...
单元、 <span style="color:blue">发起方交易员一债通账户、银行间托管账号、</span>
...
<span style="color:blue">N</span>
<span style="color:blue">C11</span>
<span style="color:blue">托管账</span>
<span style="color:blue">452</span>
<span style="color:blue">PartyRole</span>
<span style="color:blue">取</span> <span style="color:blue">28</span> <span style="color:blue">，表示当前</span> <span style="color:blue">PartyID</span> <span style="color:blue">的取值为银行间托管账号</span>
<span style="color:blue">N</span>
...
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">对手方投资者账户或三方回购专户</span>
<span style="color:blue">N</span>
<span style="color:blue">C13</span>
<span style="color:blue">投资者</span>
<span style="color:blue">452</span>
<span style="color:blue">PartyRole</span>
<span style="color:blue">取</span> <span style="color:blue">39</span> <span style="color:blue">，表示当前</span> <span style="color:blue">PartyID</span> <span style="color:blue">的取值为对手方投资者账户</span>
<span style="color:blue">N</span>
<span style="color:blue">N4</span>
<span style="color:blue">账户</span>
<span style="color:blue">对手方</span>
<span style="color:blue">448</span>
<span style="color:blue">PartyID</span>
<span style="color:blue">对手方账户名称，支持中文。</span>
<span style="color:blue">N</span>
<span style="color:blue">C180</span>
<span style="color:blue">证券账</span>
<span style="color:blue">取</span> <span style="color:blue">36</span> <span style="color:blue">，表示当前</span> <span style="color:blue">PartyID</span> <span style="color:blue">的取值为对手方投资者账户</span>
<span style="color:blue">452</span>
<span style="color:blue">PartyRole</span>
<span style="color:blue">N</span>
<span style="color:blue">N4</span>
...
<span style="color:blue">600251</span>
<span style="color:blue">债券回售撤销</span>
1. <span style="color:blue">对转股</span> <span style="color:blue">/</span> <span style="color:blue">换股和</span>
...
2. <span style="color:blue">对三方回购转</span>
...
<span style="color:blue">者账户；</span>
107
...
<span style="color:blue">填</span>
<span style="color:blue">TimeInForce</span> <span style="color:blue">必填。</span>
<span style="color:blue">号</span> <span style="color:blue">/</span> <span style="color:blue">投资者账户</span> <span style="color:blue">/</span> <span style="color:blue">证券</span>
<span style="color:blue">600180</span>
<span style="color:blue">确定报价</span>
<span style="color:blue">账户名称有效</span> <span style="color:blue">*</span> <span style="color:blue">。</span>
<span style="color:blue">/</span>
<span style="color:blue">/</span>
<span style="color:blue">/</span>
<span style="color:blue">600190</span>
...
452
PartyRole
取 5 ，表示当前 PartyID 的取值为发起方投资者 <span style="color:blue">帐户账户</span> 。 Y
N4
110
...
消息数据错误
Message Data Error
注：本表仅提供交易网关错误码，系统后台错误码参照 <span style="color:blue">每日发送的</span> 互联网交易平台 <span style="color:blue">/</span> <span style="color:blue">新固定收益系统</span> 错
误码信息文件。
118
...
<metadata>
{
  "title": "20260612_IS122_上海证券交易所交易网关STEP接口规格说明书（互联网交易平台）2",
  "source_url": null,
  "raw_path": "knowledge\\raw\\sse\\技术接口\\20260612_IS122_上海证券交易所交易网关STEP接口规格说明书（互联网交易平台）2.00版_20260612.pdf",
  "markdown_path": "knowledge\\articles\\sse\\markdown\\技术接口\\IS122_上海证券交易所交易网关STEP接口规格说明书（互联网交易平台）2.00版_20260612.md",
  "file_hash": "sha256:8ba034c3e609a5437ee0fc683757e15072bdaf98f26a2719b90429cf9fc8213b",
  "file_format": "pdf",
  "page_count": 124,
  ...
  "parse_date": "2026-06-13T17:43:55.438750+00:00",
  "sub_category": null
}
</metadata>